...
async function generate(prompt, seed, model) {
    ...
    const url = 'https://image.pollinations.ai/prompt/' + p +
        ...
        '&nologo=true&negative=' + n + '&model=' + model;
...
// ===== РЕСТАВРАЦИЯ (Img2Img — достраивает разрушенное) =====
...
function renderPage(page) {
    ...
    if (page === 'generate') {
        ...
        c.innerHTML =
            ...
            '<p style="color:var(--text-muted);margin-bottom:20px">Загрузите фото разрушенного здания. ИИ достроит утраченные части.</p>' +
            '<div class="form-grid">' +
                '<div class="form-group full-width"><label>Фото здания</label><input type="file" id="rf" accept="image/*"></div>' +
                '<div class="form-group full-width"><label>Что восстановить?</label><textarea id="rp" placeholder="Например: достроить крышу, восстановить окна, убрать трещины, вернуть лепнину..."></textarea></div>' +
        ...
        if (!S.history.length) {
            ...
            S.history.forEach((item, idx) => {
                ...
                d.innerHTML =
                    '<img src="' + item.url + '" loading="lazy" onclick="openLB(' + idx + ')">' +
        ...
        c.innerHTML =
            '<h2 class="page-title">🧠 Обучение</h2>' +
        ...
        c.innerHTML =
            ...
            'Реставрация использует img2img — ИИ достраивает разрушенные части по описанию.<br>' +
    ...
    a.innerHTML =
        '<div class="result">' +
            ...
            '<div style="margin-bottom:15px;color:var(--text-muted);font-size:.9rem">' +
                'Стиль: <b>' + data.style + '</b> | Движок: <b>' + data.engine + '</b>' +
            '</div>' +
...
function showError(msg) {
    const a = document.getElementById('res-area');
    a.innerHTML =
        '<div class="result" style="border-color:var(--error)">' +
...
function openLB(idx) {
    ...
    box.innerHTML =
        ...
        '<div style="margin-top:10px;color:var(--text-muted);font-size:.85rem">' +
...
async function doGen(prompt, style) {
    ...
    try {
        ...
        if (S.engine === 'horde') {
            ...
            alert('Движок Canvas только для офлайн-обработки.\nВыберите Pollinations или AI Horde.');
            hideLoad();
            return;
        } else {
